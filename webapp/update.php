...
// movieDB 데이터베이스 접속
$db = mysqli_connect("localhost", "movie_user", "1234", "mobieDB")
    or die("movieDB 데이터베이스 접속 실패!");

$title = $_POST['stitle'];

$sql = "SELECT * FROM movie WHERE title='" . $tittle . "';";
$result = mysqli_query($db, $sql);

if($result) {
    $coubt = mysqli_num_rows($result);
    if ($count == 0){
		...
		<!-- 영화 정보 추가 화면 -->
		 <main data-role="page" id="page2">
			<header data-role="header" data-position="fixed" data-theme="b">
                ...
                <h1>영화 정보 수정</h1>
				<a href="index.php" data-icon="home" data-iconpos="notext" class="ui-brn-right-">영화목록 (home)</a>
            
                <nav data-role="navbar">
                    <ul>
                        <li><a href="./insert.php" >입력 </a></li>
					    <li><a href="./updata_select.php" class="ui-btn-active">수정</a></li>
				...
				<form name="form2" method="POST" action="update_result.php" data-ajax="false">
                    <div class="ui-body ui-body-a">
                        <label>장로: </label>
                            ...
                            <label>영화명: </label>
                            <input type="text" name="title" id="title" value="<?php echo $title; ?>" data-inline="true">       
                            <label>상영날짜 (yyyy-mm-dd): </label>
                            <input type="data" name="data" id="data" value="<?php echo $date; ?>" data-mini="true data-inline="true">
                            <label>관람료: </label>
